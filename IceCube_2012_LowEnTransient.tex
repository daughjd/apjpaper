...
The search methods employed in the analysis of this data are nearly identical to those used in previous time-dependent IceCube analyses (see \cite{?} and \cite{?}). The arrival times and directions of events within the dataset are fed to a likelihood function which is then used to compare a signal plus background hypothesis of the data to a background only hypothesis.

Construction of this likelihood function begins with the assignment of individual event probabilities that reflect the likelihood of seeing an event $i$ arriving at $t_o$ with reconstructed direction $x_i$ given a hypothetical source located at $\vec{x}_s$ with strength $n_s$ and some time profile.
\begin{equation}\label{eq:EventProb}
\mathcal{P}_i(\vec{x}_i,t_i|\vec{x}_s,n_s,T_o) = \frac{n_s}{n_{\mathrm{tot}}} \mathcal{S}_i + \left(1-\frac{n_s}{n_{\mathrm{tot}}}\right) \mathcal{B}_i
\end{equation}
...
\end{equation}
The spatial term $S_i$ is the Kent-Fisher distribution, and it is analogous to the Gaussian distribution normalized to the 2-sphere \cite{Fisher_Bingham}.
